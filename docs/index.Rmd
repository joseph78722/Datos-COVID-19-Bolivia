...
author: '**Elaboración**: José Romero      **Fuente**: Ministerio de Salud     **Actualizado**: 17-05-2020'
output: 
  flexdashboard::flex_dashboard:
    logo: "C:\\Users\\Asus\\Documents\\Datos COVID-19 Bolivia\\icono3.png"
    Favicon: "C:\\Users\\Asus\\Documents\\Datos COVID-19 Bolivia\\icono3.png"
...
valor2 <- max(Rec$Recuperados, na.rm = FALSE)
...
Fall <- read_csv("C:\\Users\\Asus\\Documents\\Datos COVID-19 Bolivia\\data\\Fallecidos.csv", 
                       col_types = cols(Fallecidos = col_integer(), 
                                        Fecha = col_date(format = "%d/%m/%Y")))
Valor3 <- max(Fall$Fallecidos, na.rm = FALSE)
...
nuevos_casos <- ggplotly(nuevos_casos) %>% config ( displayModeBar  =  F ) %>% layout(xaxis=list(fixedrange=TRUE)) %>% 
  layout(yaxis=list(fixedrange=TRUE))  
nuevos_casos
